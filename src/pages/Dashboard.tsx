...
const areaData = [
  { name: 'Tributário',     value: 35, color: '#DEC078' },
...
const urgMap: Record<string, { bg: string; text: string; label: string }> = {
  critico: { bg: 'rgba(225,29,72,0.10)',   text: '#e11d48', label: 'Crítico'  },
  alta:    { bg: 'rgba(251,146,60,0.12)',  text: '#ea580c', label: 'Alta'     },
  media:   { bg: 'rgba(234,179,8,0.12)',   text: '#a16207', label: 'Média'    },
  baixa:   { bg: 'rgba(8,145,178,0.12)',   text: '#0e7490', label: 'Baixa'    },
};
const statusMap: Record<string, { bg: string; text: string; label: string }> = {
  ativo:            { bg: 'rgba(0,179,126,0.12)',   text: '#065f46', label: 'Ativo'         },
  recurso:          { bg: 'rgba(222,192,120,0.16)', text: '#8C6B0A', label: 'Recurso'       },
  recurso_especial: { bg: 'rgba(124,58,237,0.12)',  text: '#5b21b6', label: 'Rec. Especial' },
};
...
function SectionTitle({ icon: Icon, iconColor, children }: {
  ...
  return (
    <div className="flex items-center gap-2 mb-4">
      ...
      <span className="font-bold text-sm font-sans" style={{ color: '#222222' }}>{children}</span>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4 rounded-2xl px-6 py-5 relative overflow-hidden"
        style={{
          background: 'linear-gradient(135deg, #C8A052 0%, #DEC078 40%, #F0D090 70%, #C8A052 100%)',
          border: '1px solid rgba(222,192,120,0.60)',
          boxShadow: '0 4px 32px rgba(222,192,120,0.25)',
        }}>
        {/* brilho decorativo */}
        <div className="absolute inset-0 pointer-events-none"
          style={{ background: 'radial-gradient(ellipse at 20% 50%, rgba(255,255,255,0.20) 0%, transparent 60%)' }} />
        <div>
          <h1 className="text-2xl font-bold font-serif" style={{ color: '#19385C', letterSpacing: '-0.01em' }}>
            Dashboard Executivo
          </h1>
            ...
            <div key={item.label} className="rounded-2xl p-4 flex flex-col gap-3"
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold mt-1 font-serif" style={{ color: '#222222', letterSpacing: '-0.02em' }}>{item.value}</p>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-sm font-bold font-sans" style={{ color: item.up ? '#00b37e' : '#e11d48' }}>{item.change}</span>
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
        {[
          { label: 'Gestão Jurídica', icon: Scale,      color: '#19385C' },
          { label: 'Operações',       icon: Activity,   color: '#00b37e' },
          { label: 'Setor Público',   icon: Building2,  color: '#7c3aed' },
          { label: 'Processos',       icon: Gavel,      color: '#DEC078' },
          { label: 'Financeiro',      icon: DollarSign, color: '#00b37e' },
          { label: 'Dr. Ben IA',      icon: Brain,      color: '#7c3aed' },
        ].map(m => {
          const Icon = m.icon;
          return (
            <div key={m.label}
              className="rounded-xl p-4 cursor-pointer text-center group transition-all hover:-translate-y-0.5"
      ...
      <div className="rounded-2xl overflow-hidden" style={card}>
        ...
        {alertas.map((a, i) => {
          ...
          const cfg: Record<string, { accent: string; areaBg: string; areaColor: string }> = {
            danger:  { accent: '#e11d48', areaBg: 'rgba(225,29,72,0.08)',  areaColor: '#e11d48' },
            warning: { accent: '#f59e0b', areaBg: 'rgba(245,158,11,0.10)', areaColor: '#b45309' },
            info:    { accent: '#19385C', areaBg: 'rgba(25,56,92,0.08)',   areaColor: '#19385C' },
          };
          const c = cfg[a.tipo];
          return (
            <div key={i}
              className="flex items-center gap-3 px-5 py-3 cursor-pointer transition-colors hover:bg-gray-50"
              style={{ borderBottom: i < alertas.length - 1 ? '1px solid #F3F4F6' : 'none',
                       borderLeft: `3px solid ${c.accent}` }}>
              <Icon size={15} style={{ color: c.accent, flexShrink: 0 }} />
              <span className="text-sm flex-1 font-sans" style={{ color: '#222222' }}>{a.msg}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <AreaChart data={revenueData}>
              <defs>
                <linearGradient id="rec" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%"  stopColor="#19385C" stopOpacity={0.25} />
                  <stop offset="95%" stopColor="#19385C" stopOpacity={0} />
                </linearGradient>
              ...
              <Area type="monotone" dataKey="receita" stroke="#19385C" strokeWidth={2.5} fill="url(#rec)"  name="Receita" />
        ...
        <div className="rounded-2xl p-5" style={card}>
          <SectionTitle icon={Scale} iconColor="#DEC078">Processos por Área</SectionTitle>
          ...
          <div className="space-y-1.5 mt-2">
            {areaData.map(a => (
              <div key={a.name} className="flex items-center gap-2 text-xs">
                ...
                <span className="flex-1 font-sans" style={{ color: '#6B7280' }}>{a.name}</span>
                <span className="font-bold" style={{ color: '#222222' }}>{a.value}%</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="rounded-2xl p-5" style={card}>
        ...
        <ResponsiveContainer width="100%" height={150}>
          <BarChart data={setorData} barSize={30}>
            ...
            <XAxis dataKey="setor" tick={{ fill: '#9CA3AF', fontSize: 11 }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fill: '#9CA3AF', fontSize: 11 }} axisLine={false} tickLine={false} />
            <Tooltip contentStyle={tooltipStyle} />
            <Bar dataKey="processos" radius={[6, 6, 0, 0]}>
              {setorData.map((_, i) => (
                <Cell key={i} fill={['#DEC078','#19385C','#00b37e','#f59e0b','#e11d48','#7c3aed'][i % 6]} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      ...
      <div className="rounded-2xl overflow-hidden" style={card}>
        ...
        <div className="px-5 py-3.5 flex items-center gap-2"
          style={{ borderBottom: '1px solid #F3F4F6', background: '#F9FAFB' }}>
          <Gavel size={16} style={{ color: '#DEC078' }} />
          <span className="font-bold text-sm font-sans" style={{ color: '#222222' }}>Processos em Andamento — Mais Urgentes</span>
          <span className="ml-auto text-xs font-semibold cursor-pointer font-sans" style={{ color: '#19385C' }}>
            Ver todos →
          </span>
        </div>
              ...
              <tr style={{ borderBottom: '1px solid #F3F4F6' }}>
                {['Nº Processo', 'Cliente', 'Área', 'Instância', 'Status', 'Próx. Prazo', 'Urgência'].map(h => (
                  <th key={h} className="text-xs font-bold uppercase px-4 py-3 text-left font-sans"
                    style={{ color: '#19385C', letterSpacing: '0.07em', background: '#F9FAFB' }}>
                    {h}
                  </th>
                ))}
              </tr>
              ...
              {processosList.map((p, i) => {
                ...
                return (
                  <tr key={i}
                    ...
                    style={{ borderBottom: i < processosList.length - 1 ? '1px solid #F9FAFB' : 'none' }}
                    onMouseEnter={e => (e.currentTarget.style.background = '#F9FAFB')}
                    onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                    <td className="px-4 py-3 text-sm font-mono font-bold" style={{ color: '#19385C' }}>{p.num}</td>
                    <td className="px-4 py-3 text-sm font-semibold font-sans" style={{ color: '#222222' }}>{p.cliente}</td>
                    <td className="px-4 py-3 text-sm font-sans" style={{ color: '#6B7280' }}>{p.area}</td>
                    <td className="px-4 py-3 text-sm font-sans" style={{ color: '#6B7280' }}>{p.instancia}</td>
                    <td className="px-4 py-3">
                      <Badge bg={sta.bg} text={sta.text} label={sta.label} />
                    </td>
                    <td className="px-4 py-3 text-sm font-bold font-sans" style={{ color: '#222222' }}>{p.prazo}</td>
                    <td className="px-4 py-3">
        ...
        {[
          ...
          return (
            <div key={s.label} className="rounded-2xl p-4 flex items-center gap-4"
              style={{ ...card, borderLeft: `3px solid ${s.accent}` }}>
              <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                style={{ background: `${s.accent}18` }}>
                <Icon size={20} style={{ color: s.accent }} />
              </div>
              <div>
                <div className="text-2xl font-bold font-serif" style={{ letterSpacing: '-0.02em', color: '#222222' }}>{s.value}</div>
                <div className="text-xs font-semibold font-sans mt-0.5" style={{ color: '#6B7280' }}>{s.label}</div>
              </div>
            </div>
          );
        })}
      ...
      <div className="rounded-2xl p-5" style={{ ...card, background: '#F9FAFB' }}>
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center shadow-md"
            style={{ background: 'linear-gradient(135deg, #C8A052, #F0D090)', border: '1px solid rgba(222,192,120,0.50)' }}>
            <Brain size={20} style={{ color: '#19385C' }} />
          </div>
          <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {[
            ...
            <div key={a.label}
              className="rounded-xl p-3 flex items-center gap-2.5 cursor-pointer transition-all group hover:brightness-110"
